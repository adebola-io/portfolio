export const works = [
   {
      name: "Siphon",
      logo: "images/logos/siphon.png",
      logoFilter: "saturate(0) brightness(0) invert(1) opacity(0.25)",
      link: "https://www.npmjs.com/package/siphon-cli",
      code: "http://github.com/adebola-io/siphon",
      featured: true,
      info: "A simple Vanilla code bundler.",
      thColor: "#cf7007",
      thColor2: "#523602",
      technologies: ["NodeJS", "Typescript"],
   },
   {
      name: "Aura",
      logo: "images/logos/aura.png",
      logoFilter: "saturate(0) contrast(1.5) brightness(0.5) opacity(0.5)",
      link: "http://github.com",
      code: "http://github.com",
      featured: true,
      info: "Aura gives you a glimpse into the future, enlightened insights and advice on your life choices (Take seriously at your own discretion).",
      thColor: "#5011c1",
      thColor2: "#600078",
      // scColor: "#000000",
      technologies: ["Typescript", "Tailwind"],
   },
   {
      name: "Sojourner",
      logo: "images/logos/sojourner.png",
      logoFilter: "saturate(0) invert(1) brightness(0.8) opacity(0.5)",
      link: "http://sojourner.netlify.app",
      code: "http://bubble-music.netlify.app",
      featured: true,
      info: "A website for Sojourner, an interstellar tourism agency founded in the year 2075.",
      thColor: "#191969",
      thColor2: "#294ba9",
      // scColor: "#1b033a",
      technologies: ["Vue"],
   },
   {
      name: "Portfolio",
      logo: "images/logos/motion.png",
      logoFilter: "saturate(0) brightness(0.5) opacity(0.5)",
      link: "http://adebola-xyz.netlify.app",
      code: "http://github.com/adebola-xyz/portfolio",
      featured: true,
      info: "The site you're on right now.",
      thColor: "#000000",
      thColor2: "#151515",
      technologies: ["React", "Redux", "SCSS"],
   },
   {
      name: "Harmony",
      logo: "images/logos/harmony.png",
      logoFilter: "saturate(0) invert(1) brightness(0.6) opacity(0.5)",
      link: "http://github.com",
      code: "http://github.com",
      info: "A book store.",
      thColor: "#ef3232",
      thColor2: "#a30b25",
      technologies: ["React", "Redux"],
   },
   {
      name: "Pageweaver",
      logo: "images/logos/pageweaver.png",
      logoFilter: "saturate(0) brightness(0.7) opacity(0.5)",
      link: "http://pageweaver.netlify.app",
      code: "http://bubble-music.netlify.app",
      info: "A command-line application for auto-generating new web projects.",
      featured: true,
      thColor: "#05461b",
      thColor2: "#0d5c03",
      technologies: ["NodeJS"],
   },
];

export const stack = [
   {
      name: "React",
      logo: "images/logos/react.png",
      filter: "sepia(1) saturate(4.5)",
      percent: 87,
   },
   {
      name: "Javascript",
      logo: "images/logos/js.png",
      filter: "none",
      percent: 100,
   },
   {
      name: "Vue",
      logo: "images/logos/vue.png",
      filter: "invert(1) saturate(0) sepia(1) saturate(5.5)",
      percent: 65,
   },
   {
      name: "Typescript",
      logo: "images/logos/ts.png",
      filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
      percent: 82,
   },
   {
      name: "Tailwind",
      logo: "images/logos/tailwind.png",
      filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
      percent: 59,
   },
   {
      name: "CSS-in-JS",
      logo: "images/logos/css.png",
      filter: "invert() sepia(1) saturate(4.5) hue-rotate(8deg)",
      percent: 100,
   },
   {
      name: "SCSS",
      logo: "images/logos/sass.png",
      filter:
         "invert() sepia(1) saturate(8.5) hue-rotate(26deg) brightness(1.25)",
      percent: 92,
   },
   {
      name: "NPM",
      filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
      percent: 85,
   },
   {
      name: "Node JS",
      logo: "images/logos/node.png",
      filter:
         "invert(1) sepia(1) contrast(1.5) saturate(5.5) hue-rotate(-6deg)",
      percent: 95,
   },
   // {
   //   name: "Wordpress",
   //   logo: require("images/logos/wordpress.png"),
   //   filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
   // },
   {
      name: "MySQL",
      logo: "images/logos/mysql.png",
      filter: "invert(1) sepia(1) saturate(5.5)",
   },
   {
      name: "Redux",
      logo: "images/logos/redux.png",
      filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
   },
   // {
   //   name: "MongoDB",
   //   logo: require("images/logos/mongo.png"),
   //   filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
   // },
   {
      name: "Angular",
      logo: "images/logos/angular.png",
      filter: "invert(1) sepia(1) saturate(6) hue-rotate(7deg)",
   },
];

export const contacts = [
   {
      name: "Twitter",
      link: "https://twitter.com/adebola_io",
      logo: "images/logos/twitter.png",
      imgFilter: "saturate(0) brightness(2)",
      hoverFilter: "sepia(1) saturate(5) hue-rotate(6deg)",
      footerImgFilter: "saturate(0) brightness(0.5)",
   },
   {
      name: "Github",
      link: "https://github.com/adebola-io",
      logo: "images/logos/github.png",
      imgFilter: "saturate(0) brightness(2)",
      hoverFilter: "sepia(1) saturate(5) hue-rotate(-3deg)",
      footerImgFilter: "saturate(0) brightness(0.5)",
   },
   {
      name: "LinkedIn",
      link: "https://www.linkedin.com/in/oluwasefunmi-akomolafe-3a6a42214/",
      logo: "images/logos/linkedin.png",
      imgFilter: "saturate(0) invert(1) brightness(2)",
      hoverFilter: "invert(1) sepia(1) saturate(53) hue-rotate(-27deg)",
      footerImgFilter: "saturate(0) opacity(0.25) invert(1)",
   },
];

export const circlesAnimations = [
   {
      name: "around_screen",
      keyframes: [
         { transform: "translate(-50%, -50%)" },
         { transform: "translate(50%, 50%)" },
         { transform: "translate(100%, -50%)" },
         { transform: "translate(150%, 50%)" },
         { transform: "translate(200%, -50%)" },
         { transform: "translate(200%, 50%)" },
         { transform: "translate(150%, -50%)" },
         { transform: "translate(100%, 50%)" },
         { transform: "translate(50%, -50%)" },
         { transform: "translate(0%, 50%)" },
         { transform: "translate(-50%, -50%)" },
      ],
      duration: 15000,
      iteration: Infinity,
   },
   {
      name: "around_screen",
      keyframes: [
         { transform: "translate(-50%, -50%)" },
         { transform: "translate(-50%, 50%)" },
         { transform: "translate(100%, -50%)" },
         { transform: "translate(200%, 50%)" },
         { transform: "translate(200%, -50%)" },
         { transform: "translate(100%, 50%)" },
         { transform: "translate(100%, -50%)" },
         { transform: "translate(-50%, 50%)" },
         { transform: "translate(-50%, -50%)" },
      ],
      duration: 15000,
      iteration: Infinity,
   },
];
